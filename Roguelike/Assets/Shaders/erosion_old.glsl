...
void get_outflows(ivec2 coord, out float[8] water_outflows, out float[8] sediment_outflows, out vec4 eroded_values) {
    ...
    float current_altitude = bedrock_height + sediment_depth + water_depth + suspended_sediment;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float neighbor_altitude = neighbor_data.r + neighbor_data.g + neighbor_data.b + neighbor_data.a;

        float height_difference = (current_altitude - neighbor_altitude) / 2;
    ...
    float expected_outflow = max(0, min(total_height_differences, water_depth + sediment_depth));

    // Deposite sediment
    float sedimentation = suspended_sediment * (1 - min(1, expected_outflow / zero_flow_rate));
    sediment_depth += sedimentation;
    suspended_sediment -= sedimentation;

    // Erode sediment
    float erosion = expected_outflow * erosion_factor;
    ...
    // The total amount of water + suspended sediment that flows to neighbors should equal the expected_outflow
    // The actual amount of water and sediment should be proportinal to the ratio of water depth to suspended sediment
    if (water_depth + suspended_sediment > 0) {
        float water_ratio = water_depth / (water_depth + suspended_sediment);
        float expected_water_outflow = water_ratio * expected_outflow;
        float expected_sediment_outflow = (1 - water_ratio) * expected_outflow;
        ...
        if (total_height_differences > 0) {
            for (int i = 0; i < 8; i++) {
                ...
                sediment_outflows[i] = expected_sediment_outflow * outflow_share;
