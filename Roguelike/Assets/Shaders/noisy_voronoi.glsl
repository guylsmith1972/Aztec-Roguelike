...
float distance_to_seed(vec2 frag_coord, vec2 seed_pos, float weight, float noisiness) {
    ...
    return sqrt(dot(diff, diff)) * weight * (1.0 - noise_value * noisiness);
}
...
void real_main() {
   float min_distance = 1e9; // Arbitrarily large number
    ...
    for (int i = 0; i < seed_count; ++i) {
        float distance = distance_to_seed(gl_GlobalInvocationID.xy, seeds[i].xy, seeds[i].z, seeds[i].w);
    ...
    imageStore(output_image, ivec2(gl_GlobalInvocationID.xy), vec4(float(seed_index), 0.0, 0.0, 0.0));
}
