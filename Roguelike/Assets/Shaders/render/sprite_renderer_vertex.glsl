...
layout(location = 2) in vec2 in_instance_position;  // Instanced position of sprite in world coordinates. Pixel coordinates are world coordinates * sprite_size_in_pixels
layout(location = 3) in float in_spritesheet_index; // Instanced spritesheet index

uniform vec2 sprite_size_in_pixels;             // Width and height of sprite in pixels
uniform vec2 spritesheet_dimensions_in_sprites; // Width and height of spritesheet, measured in sprites
...
void main()
{
    ...
    vec2 adjusted_position = (in_instance_position - camera_position_in_world) * sprite_size_in_pixels;

    // Scale and translate the in_position to fit the sprite's size
    vec2 sprite_position = adjusted_position + in_position * sprite_size_in_pixels;
    ...
    frag_uv = in_uv;
    frag_spritesheet_index = in_spritesheet_index;
}
